...
from fastapi_zero.core.security import JWT, extract_token_header
...
async def get_user(
    ...
    token: JWT = Depends(extract_token_header),
):
    return await UserService.list_user(
        limit=limit, offset=offset, db=db, token=token
    )
...
async def get_user_by_id(
    ...
    return await UserService.get_user_by_id(id=id, db=db, token=token)
...
async def update_user(
    ...
    await UserService.update_user(user_update=user, db=db, token=token)
...
async def get_user_addresses(
    ...
    return await UserService.get_user_address(id=id, db=db, token=token)
...
async def create_user_address(
    ...
    return await UserService.create_address_user(
        address_create=address, db=db, token=token
    )
...
async def update_user_address(
    ...
    await UserService.update_address(
        id=id_address, address_update=address, db=db, token=token
    )
...
async def delete_user_address(
    ...
    await UserService.delete_address(id=id_address, db=db, token=token)
